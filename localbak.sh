...
# | Usage:
# |      1 - bash localbak.sh "backup contents"
# |      2 - bash localbak.sh
# *********************************************************************************************
...
# Function Description: Create a tar.gz backup of the project directory.
#
# Args:
#    - param: Optional suffix string to append to the backup filename.
...
function backup_project() {
    ...
    local backup_time="$(date '+%F-%H-%M-%S')"
    local backup_cnt="${1}"
...
#
# Args:
#    - param: Optional suffix string for the backup filename.
